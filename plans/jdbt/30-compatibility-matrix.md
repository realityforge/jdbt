...
| Import command | `runtime.rb#database_import` | `import` | done | Runtime flow and CLI wiring implemented with --resume-at behavior, standalone import-module ordering, per-module delete sequencing, and failure guidance; PostgreSQL cross-database standard import requires explicit SQL files. |
...
| Repository config parsing | `repository_definition.rb#from_yaml` | `RepositoryConfigLoader` | done | Supports Ruby `!omap` tagged modules, omap-style lists, and map-style module declarations. |
...
| Schema hash | `runtime.rb#database_schema_hash`, `#hash_files` | Runtime loader hash | done | Hashes ordered artifact paths plus MD5 content for SQL, fixtures, import files, hooks, and migrations. |

## Database Drivers

| Area | Ruby Source | Java Target | Status | Notes |
| --- | --- | --- | --- | --- |
| SQL Server driver | `drivers/dialect/sql_server.rb` | `SqlServerDbDriver` | done | JDBC-backed execution path with migration/import support, version metadata, SQL Server drop controls, optional data/log file placement, reindex maintenance, shrink maintenance, and backup-history deletion defaults. |
...
| Module-group delete ordering | TODO noted in `test_runtime_basic.rb` | All deletes before imports for module-group imports only | intentional_divergence | Planned behavioral fix; standalone import follows Ruby import definition order and deletes per module before importing that module. |
